...
void ASViewCharacter::SetViewMode(EViewMode InViewMode)
{
	...
	case EViewMode::QuarterView:
		...
		bUseControllerRotationPitch = false;
		bUseControllerRotationYaw = false; // true하면 방향 휙휙 바뀜
		bUseControllerRotationRoll = false;
		...
		GetCharacterMovement()->RotationRate = FRotator(0.f, 480.f, 0.f); // 이걸로 회전 보간
		break;
...
void ASViewCharacter::Tick(float DeltaSeconds)
{
	...
	{
		SpringArmComponent->TargetArmLength = FMath::FInterpTo(SpringArmComponent->TargetArmLength, DestArmLength, DeltaSeconds, ArmLengthChangeSpeed); //보간
